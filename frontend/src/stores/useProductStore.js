...
export const useProductStore = create((set) => ({
	...
	createProduct: async (productData) => {
		set({ loading: true });
		try {
			const res = await axiosInstance.post("/products/addProduct", productData);
			set((prevState) => ({
	...
	fetchAllProducts: async () => {
		...
		try {
			...
			toast.error("Fail to fetch products, refresh or try again later")
		}
	},
	...
	fetchProductsByCategory: async (category) => {
		...
		try {
			...
			set({ error: "Failed to fetch products", loading: false });
			console.log(error.response.data.error || "Failed to fetch products");
			toast.error("Fail to fetch products categories, refresh or try again later")
		}
	},
